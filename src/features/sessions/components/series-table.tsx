...
export function WeeklySeriesTable({ series, onCancelSeries }: WeeklySeriesTableProps) {
  ...
  async function cancelSeries(item: WeeklySeriesItem) {
    const scope = scopes[item.id] || 'ENTIRE_SERIES';
    const effectiveOccurrenceId = scope === 'ENTIRE_SERIES' ? undefined : item.nextOccurrence?.id;
    if (!effectiveOccurrenceId && scope !== 'ENTIRE_SERIES') {
      setError('No future occurrence is available for this cancellation scope.');
      return;
    }
    if (!window.confirm(scope === 'THIS' ? 'Cancel only the next occurrence?' : scope === 'THIS_AND_FUTURE' ? 'Cancel this and all future occurrences?' : 'Cancel this entire series and its future occurrences?')) return;
    setPendingId(item.id);
    setError(null);
    try {
      await onCancelSeries(item.id, scope, effectiveOccurrenceId);
    } catch (actionError) {
          ...
          {series.map((item) => (
            <article key={item.id} className="flex flex-col gap-4 px-4 py-5 sm:px-5 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap gap-2 lg:justify-end">
                ...
                {item.status === 'ACTIVE' && <><Combobox options={[{ value: 'THIS', label: 'This occurrence' }, { value: 'THIS_AND_FUTURE', label: 'This and future' }, { value: 'ENTIRE_SERIES', label: 'Entire series' }]} value={scopes[item.id] || 'ENTIRE_SERIES'} onChange={(value) => setScopes((current) => ({ ...current, [item.id]: value as RecurrenceScope }))} placeholder="Cancel scope" searchPlaceholder="Search scopes" className="min-w-[190px]" /><button type="button" disabled={pendingId === item.id} onClick={() => void cancelSeries(item)} className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-lg border border-rose-200 px-3.5 py-2 text-sm font-semibold text-rose-700 hover:bg-rose-50 disabled:opacity-50"><XCircle className="h-4 w-4" aria-hidden="true" /> {pendingId === item.id ? 'Cancelling…' : 'Cancel'}</button></>}
              </div>
            </article>
          ))}
        </div>
